-- Seed next two complete Mon–Fri weeks for Resource Planner + Confirmations.
-- Uses live employees / projects / milestones / activities (activity_id FK).
-- Idempotent for reason = 'seed:next-2-weeks'.
-- Relative to the app’s current working week (ISO Monday of CURRENT_DATE):
--   Week 1 = this_monday + 7 .. + 11
--   Week 2 = this_monday + 14 .. + 18
-- Around 2026-07-20 that is 2026-07-27–31 and 2026-08-03–07.
--
-- Run (PowerShell):
--   Get-Content -Raw scripts\seed-next-2-weeks.sql | docker compose exec -T postgres psql -U admin -d oneview -v ON_ERROR_STOP=1
...
SELECT
  (this_monday + 7)  AS w1_start,
  (this_monday + 11) AS w1_end,
  (this_monday + 14) AS w2_start,
  (this_monday + 18) AS w2_end
FROM bounds;
...
WHERE reason = 'seed:next-2-weeks'
...
SELECT employee_id, project_id, milestone_id, activity_id, tasks,
       start_date, end_date, hours_per_day, 'seed:next-2-weeks',
...
FROM (
  ...
  SELECT s.anil AS employee_id, s.amul AS project_id, s.amul_ms AS milestone_id, s.act_feature AS activity_id,
         ARRAY['Next sprint APIs']::text[] AS tasks, w.w1_start AS start_date, w.w1_end AS end_date, 5.0::float8 AS hours_per_day
  FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.digant, s.amul, s.amul_ms, s.act_feature, ARRAY['Planner polish'], w.w1_start, w.w1_end, 6.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.amit, s.skyview, s.sky_ms, s.act_design, ARRAY['SkyView flows'], w.w1_start, w.w1_end, 5.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.persist_emp, s.persist_prj, s.persist_ms, s.act_review, ARRAY['PR backlog'], w.w1_start, w.w1_end, 4.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.test_emp, s.persist_prj, s.persist_ms, s.act_support, ARRAY['L2 queue'], w.w1_start, w.w1_end, 6.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.ravi, s.amul, s.amul_ms, s.act_qa, ARRAY['Sprint tests'], w.w1_start, w.w1_end, 5.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  ...
  UNION ALL
  SELECT s.digant, s.skyview, s.sky_ms, s.act_docs, ARRAY['Release draft'], w.w1_start, w.w1_end, 2.0 FROM seed_ids s, seed_weeks w
  -- Week 2 (week after next)
  UNION ALL
  SELECT s.anil, s.amul, s.amul_ms, s.act_bug, ARRAY['Stabilization'], w.w2_start, w.w2_end, 4.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.digant, s.amul, s.amul_ms, s.act_feature, ARRAY['Confirmations UX'], w.w2_start, w.w2_end, 6.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.amit, s.amul, s.amul_ms, s.act_design, ARRAY['Amul refresh'], w.w2_start, w.w2_end, 4.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.persist_emp, s.skyview, s.sky_ms, s.act_feature, ARRAY['SkyView APIs'], w.w2_start, w.w2_end, 5.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.test_emp, s.persist_prj, s.persist_ms, s.act_support, ARRAY['Handoffs'], w.w2_start, w.w2_end, 5.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.ravi, s.skyview, s.sky_ms, s.act_qa, ARRAY['UAT pass'], w.w2_start, w.w2_end, 6.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.amit, s.skyview, s.sky_ms, s.act_meeting, ARRAY['Design sync'], w.w2_start, w.w2_end, 2.0 FROM seed_ids s, seed_weeks w
  UNION ALL
  SELECT s.digant, s.persist_prj, s.persist_ms, s.act_review, ARRAY['Peer reviews'], w.w2_start, w.w2_end, 2.0 FROM seed_ids s, seed_weeks w
) x
...
JOIN allocations a
 ...
 AND a.reason = 'seed:next-2-weeks'
...
FROM allocations WHERE reason = 'seed:next-2-weeks' AND is_deleted = false;
