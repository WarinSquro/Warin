...
| Host Nginx (IP/HTTP) | `infra/nginx/host-ip.conf` |
...
3. Reach localhost-only services on EC2 via **SSH tunnel**, e.g.  
   `ssh -i PEM -L 18025:127.0.0.1:8025 ubuntu@PUBLIC_IP`
...
|---------|---------|-----------|---------------------|-----------------|----------|-------------------|------------|-----------------|
| **Warin Web (SPA)** | React UI | Dev: `http://localhost:5173` · Preview: `http://localhost:4173` · Via Compose nginx: `http://127.0.0.1:8080/` | Host Nginx: `http://PUBLIC_IP/` (e.g. `http://13.126.64.134/`) | 5173 (Vite) · 80 (host Nginx) · 4173 (preview) | App login: `admin@acme.io` | PIN **`12345`** (seeded; hashed in DB) | `VITE_API_BASE_URL`, `APP_PUBLIC_URL`, `CORS_ORIGIN` | Built to `/opt/warin/shared/web` on EC2; source in repo root |
| **Backend API** (`oneview-api`) | NestJS REST API | Container health: via nginx `http://127.0.0.1:8080/api/v1/health` · Direct (if published): `http://127.0.0.1:3001` | `http://PUBLIC_IP/api/v1/…` (proxied) | **3001** inside container (not published to host by default) | — (JWT after login) | `JWT_SECRET`, `HMAC_PEPPER` | `API_PORT`, `DATABASE_URL`, `REDIS_URL`, `JWT_*`, `HMAC_PEPPER`, `CORS_ORIGIN`, `MAIL_*`, `APP_PUBLIC_URL`, `SMTP_ENCRYPTION_KEY` | `docker-compose.yml` → `api` · `apps/oneview-api` · `.env` |
| **API Swagger** | OpenAPI UI | `http://127.0.0.1:8080/api/docs` | `http://PUBLIC_IP/api/docs` (if exposed through host Nginx) | via 8080/80 | Same JWT as API (if secured later) | — | — | `apps/oneview-api/src/main.ts` (`api/docs`) |
...
| Service | Purpose | Local URL | Production / QA URL | Default port(s) | Username | Password / secret | Env var(s) | Config location |
|---------|---------|-----------|---------------------|-----------------|----------|-------------------|------------|-----------------|
| **PostgreSQL 16** | Primary database | Host: `127.0.0.1:15432` · In Compose network: `postgres:5432` | Same pattern on EC2 (**localhost bind**); not public | Host **15432** → container **5432** | `admin` | `admin` **(compose default)** | `DATABASE_URL`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB` | `docker-compose.yml` → `postgres` · Prisma `prisma/schema.prisma` |
...
| Service | Purpose | Local URL | Production / QA URL | Default port(s) | Username | Password / secret | Env var(s) | Config location |
|---------|---------|-----------|---------------------|-----------------|----------|-------------------|------------|-----------------|
| **Compose Nginx** (`oneview-nginx`) | Proxies `/api` → API container | `http://127.0.0.1:8080/` | Bound to **127.0.0.1:8080** on EC2; fronted by host Nginx | Host **8080** → container **80** | — | — | — | `infra/nginx/default.conf` · `docker-compose.yml` → `nginx` |
| **Host Nginx** (Ubuntu) | Serves SPA + proxies `/api` to Compose nginx | N/A (EC2/host) | `http://PUBLIC_IP/` (port **80**); TLS deferred | **80** (443 later) | — | — | — | `infra/nginx/host-ip.conf` → `/etc/nginx/sites-available/warin` |
...
| Service | Purpose | Local URL | Production / QA URL | Default port(s) | Username | Password / secret | Env var(s) | Config location |
|---------|---------|-----------|---------------------|-----------------|----------|-------------------|------------|-----------------|
| **Mailpit** | Dev/QA SMTP catcher + inbox UI | SMTP `127.0.0.1:1025` · UI `http://127.0.0.1:8025` | Localhost on EC2; tunnel for UI | **1025** (SMTP), **8025** (UI) | — | — | `MAIL_SMTP_HOST=mailpit` (in Compose API), `MAIL_SMTP_PORT`, `MAIL_FROM`, `MAIL_DRY_RUN`, `MAIL_PROVIDER` | `docker-compose.yml` → `mailpit` |
| **Product SMTP (Settings)** | Real outbound mail (Forgot PIN, etc.) | Configured in UI | Same DB-backed settings on QA/prod | Per SMTP provider | Username in Settings | Password **AES-GCM** in DB (`smtp_settings.password_encrypted`); never returned by API | `SMTP_ENCRYPTION_KEY` (or fallback `HMAC_PEPPER`) | Settings → SMTP · table `smtp_settings` · `apps/oneview-api/.../smtp-settings.service.ts` |

**Mailpit on EC2 (tunnel example):**

```bash
ssh -i PEM -L 18025:127.0.0.1:8025 ubuntu@PUBLIC_IP
# then open http://127.0.0.1:18025
```
...
| **AWS EC2** | QA/prod host | — | SSH `ubuntu@PUBLIC_IP` · App HTTP `http://PUBLIC_IP/` | **22** (SSH), **80** (HTTP) | `ubuntu` | SSH key `.pem` (laptop only; never in git) | — | AWS Console SG/NACL · key e.g. `WARIN-QA-PAIR.pem` |
| **GitHub** | Source remote | — | `https://github.com/WarinSquro/Warin.git` | 443 | GitHub user/org | PAT / SSH key (not in repo) | — | `git remote -v` |
| **Vite / npm** | Frontend tooling | `npm run dev` → `:5173` | Build-time only on EC2/laptop | 5173 / 4173 | — | — | `VITE_API_BASE_URL` | `package.json`, `vite.config.*` |
...
| API health | `http://PUBLIC_IP/api/v1/health` or on-box `curl http://127.0.0.1:8080/api/v1/health` |
| Postgres | On-box `127.0.0.1:15432` or SSH tunnel |
| Mailpit UI | SSH tunnel → `127.0.0.1:8025` |
| Secrets | `/opt/warin/shared/.env` |

---

*Generated for Warin/OneView. Update this file when ports, defaults, or hosting URLs change.*
